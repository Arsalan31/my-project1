...
function Header2() {
    ...
    useEffect(() => {
        const checkIfClickedOutside = e => {
            // If the menu is open and the clicked target is not within the menu,
            // then close the menu
            if (toggleabout1 && ref.current && !ref.current.contains(e.target)) {
            ...
            <div className="flex flex-wrap justify-between content-center items-center bg-white ">
                ...
                {(toggleMenu || screenWidth > 1023) && (
                    <div className="list-none flex flex-row p-6 order-0 lg:justify-center justify-start">
                        ...
                        <li>
                            <a className="cursor-pointer uppercase block py-2 px-2 font-bold text-xs leading-7 md:text-gray-5 text-white rounded">Get Involved</a>
                        </li>
                    </div>
                )}
